...
function updateStatus(idOrganisasi, status) {
    ...
    .then(data => {
        if (data.success) {
            // Status berhasil diperbarui, hapus baris dari tabel
            var row = document.getElementById('row_' + idOrganisasi);
            row.parentNode.removeChild(row);

            // Lakukan aksi yang diperlukan (misalnya, tampilkan notifikasi)
